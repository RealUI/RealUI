...
function ConfigBar_GUI:CreateTabList(element, tabs, direction, point, x, y)
    ...
    for k, info in ipairs(tabs) do
        ...
        elseif info.texOffset then
            tabFrames[k].icon:SetPoint("TOPLEFT", info.texOffset[1], info.texOffset[2])
            tabFrames[k].icon:SetPoint("BOTTOMRIGHT", info.texOffset[3], info.texOffset[4])
        else
        ...
        if info.texture2 then
            ...
            if info.texPosition2 then
                tabFrames[k].icon2:SetPoint("TOP", tabFrames[k], "TOP", info.texPosition2.x, info.texPosition2.y)
            elseif info.texOffset2 then
                tabFrames[k].icon2:SetPoint("TOPLEFT", info.texOffset2[1], info.texOffset2[2])
                tabFrames[k].icon2:SetPoint("BOTTOMRIGHT", info.texOffset2[3], info.texOffset2[4])
            else
                tabFrames[k].icon2:SetAllPoints(tabFrames[k])
            end
            tabFrames[k].icon2:SetTexture(info.texture2)

            local leftOfs = ((info.texCoord2[2] - info.texCoord2[1]) * .5)
            tabFrames[k].icon:SetTexCoord(info.texCoord[1], info.texCoord[2] - leftOfs, info.texCoord[3], info.texCoord[4])

            local rightOfs = ((info.texCoord[2] - info.texCoord[1]) * .5)
            tabFrames[k].icon2:SetTexCoord(info.texCoord2[1] + rightOfs, info.texCoord2[2], info.texCoord2[3], info.texCoord2[4])
        end
    end
