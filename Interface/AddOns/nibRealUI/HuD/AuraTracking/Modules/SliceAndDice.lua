...
local function AuraUpdate(self, event, unit)
    ...
    local name, duration, remaining = GetBuffDuration("player", SnDSpellName)
    local endTime
    ...
    -- Potential
    if (points > 0) then
        local potentialSnD = baseSnDDuration + ((points - 1) * gapPerComboPoint)
    ...
    if ((endTime) and (endTime >= now)) then
        ...
        -- Cooldown
        if remaining then
            self.startTime = now - (maxSnDDuration - remaining)
            self.auraDuration = maxSnDDuration
...
local function TalentUpdate(self, event, unit, initializing)
    ...
    -- Raven Spell Lists
    AuraTracking:ToggleRavenAura(false, "buff", "#"..SnDSpellID, not(self.inactive))
end

function SliceAndDice:TalentRefresh()
    TalentUpdate(self.frame, nil, nil, true)
end

function SliceAndDice:AuraRefresh()
...
function SliceAndDice:SetUpdates()
...
function SliceAndDice:SetIndicatorInfo(info)
    local f = self.frame

    local name,_,icon = GetSpellInfo(SnDSpellID)
    f.texture = icon
    SnDSpellName = name
...
function SliceAndDice:CreateIndicator()
...
function SliceAndDice:New(info)
    local Indicator = {}
...
AuraTracking:RegisterModule(SliceAndDice, "SliceAndDice")
