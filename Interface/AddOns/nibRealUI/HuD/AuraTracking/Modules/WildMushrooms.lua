...
local WildMushrooms = nibRealUI:CreateModule(MODNAME, "AceEvent-3.0")
...
local function TalentUpdate(self, event, unit, initializing)
    ...
    if nibRealUI.class ~= "DRUID" then
		...
		isValid = (GetSpecialization() == 4) and (UnitLevel("player") >= MinLevel)
    end
...
function WildMushrooms:SetIndicatorInfo(info)
    ...
    -- for config UI
    info.spell = ShroomID
    info.specs = {false, false, false, true}
