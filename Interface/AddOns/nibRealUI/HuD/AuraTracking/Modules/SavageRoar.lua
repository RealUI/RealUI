...
local _
local MODNAME = "AuraTracking_SavageRoar"
local SavageRoar = nibRealUI:CreateModule(MODNAME, "AceEvent-3.0")
...
local function AuraUpdate(self, event, unit)
    ...
    -- Potential
    if (points > 0) then
        local potentialSR = baseSRDuration + ((points - 1) * gapPerComboPoint)
        self.count:SetText(potentialSR)
        self.count:SetParent(self)
        if potentialSR == maxSRDuration then
    ...
    if ((endTime) and (endTime >= now)) then
        ...
        -- Cooldown
        if remaining then
            self.startTime = now - (maxSRDuration - remaining)
            self.auraDuration = maxSRDuration
...
local function TalentUpdate(self, event, unit, initializing)
    ...
    if (nibRealUI.class ~= "DRUID") or (UnitLevel("player") < MinLevel) or (spec ~= 2) or (form ~= 1) then
        self:Hide()
        self.inactive = true
    else
        self.inactive = false
...
function SavageRoar:AuraRefresh()
...
function SavageRoar:SetUpdates()
...
function SavageRoar:SetIndicatorInfo(info)
    ...
    -- for config UI
    info.spell = SRID
    info.specs = {false, true, false, false}
...
function SavageRoar:CreateIndicator()
...
AuraTracking:RegisterModule(SavageRoar, "SavageRoar")
