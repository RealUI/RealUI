...
local _
local MODNAME = "TargetCC"
local TargetCC = nibRealUI:NewModule(MODNAME, "AceEvent-3.0", "AceTimer-3.0")
		...
		if (unit == self.player.unit) then
			...
			if self:TargetInRange() then
				...
				-- if self.player.targetInRange then
					...
					if db.colors.useGlobal then
						color = nibRealUI:ColorDarken(nibRealUI.media.colors.blue, 0.2)
					else
...
function CastBars:UpdateInterruptibleColor(unit)
	...
	if self[unit].notInterruptible then
		if db.colors.useGlobal then
			color = nibRealUI:ColorDarken(nibRealUI.media.colors.orange, 0.2)
		else
	...
	self.db:RegisterDefaults({
		profile = {
			...
			colors = {
				...
				outOfRange =		{0.25, 0.50, 0.70, 1},
				uninterruptible =	{0.73, 0.21, 0.07, 1},
